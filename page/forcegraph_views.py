...
def forcegraph(request, page_id):
    try:
        page = Forcegraph.objects.get(pk=page_id)  # ===== get DB (from API when created) =====

        #  ===== transform to pattern for visualization standard for s-p-o =====
        # new_results = extractor_trans.transform_api(page.source)
        # page.result = new_results
        # page.save()

        #  ===== making a list of Filtering
        filtering = extractor_trans.faceted_search(page.source, page.domain_subject.domain_path)

        # ======== compare facet to display  =========
        compare_facets = page.faceted_search.select_related()
        if compare_facets:
            tmps = {}
        ...
        return render(request, 'page/forcegraph.html',
                      {'posts': page, 'filter_facets': filtering['filter_facets'],
