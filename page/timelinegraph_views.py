...
from .models import Timelinegraph
from django.http import JsonResponse
import json


def timelinegraph(request, page_id):
    try:
        project_data = Timelinegraph.objects.get(pk=page_id)
        ...
        # new_results = []
        # new_results.append(extractor_trans.nested_transformation(results, "All", date_show.split('#')[-1]))
        ...
        return render(request, 'page/timelinegraph.html', {'posts': project_data, 'new_results': project_data.result,
...
def filter_timeline(request):
    if request.method == 'POST':
        if request.is_ajax():
            ...
            new_results = [extractor_trans.nested_transformation(results, "All", request.POST.get('date_show'))]
